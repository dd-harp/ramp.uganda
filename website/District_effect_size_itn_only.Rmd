...
```{r}
remove_dists <- c("Fort Portal city",names(irs_time))
itn_time <- itn_time[!(names(itn_time) %in% remove_dists)]
itn_dist <- list()
for (i in 1:length(itn_time))
  itn_dist[[i]]<- names(itn_time)[i]
itn_dist <- itn_dist[!sapply(itn_dist, is.na)]
```
...
cleaned_dists <- sapply(itn_dist, clean_name)
...
eir_files_itn <- eir_files[matched_indices]#matched_indices]
```
...
for (i in 1:length(data_list)) {
  fitted_mod[[i]] <- readRDS(eir_files_itn[i])
}
```
