...
```{r, eval=F}
#recon_Adjumani <- reconstruct_baseline(model, prts$pfpr, prts$jdate, irs_rounds, bednet_rounds, "mean")
```

```{r, eval=F}
#saveRDS(recon_Adjumani, "recon_Adjumani.rds") 
```

```{r}
readRDS("recon_Adjumani.rds") -> recon
```


```{r}
xds_plot_PR(recon_Adjumani)
```
```{r}
baseline_Adjumani <- setup_irs(recon_Adjumani, coverage_name="multiround", coverage_opts=irs_rounds, effect_sizes_name = "simple")

baseline_Adjumani <- setup_bednets(baseline_Adjumani, coverage_name = "multiround", coverage_opts = bednet_rounds, effect_sizes_name = "lemenach")
```

```{r}
baseline_Adjumani <- xds_solve(baseline_Adjumani, 3650, 10)
```

```{r}
xds_plot_PR(recon_Adjumani)
xds_plot_PR(baseline_Adjumani, clrs = "darkred", add=T)
```
```{r}
baseline_focast <- baseline_Adjumani
...
for (i in 1:n_runs) {
  bf <- baseline_Adjumani
  bf$nHostSpecies <- 1
  bf$nVectorSpecies <- 1
  bf <- setup_other_variables(bf)
  bf <- forecast_spline_Lambda(bf, 5, x_last = 1)
  bf <- xds_solve(bf, 5475, 15)
  
  # Save just the outputs (e.g., prevalence or any time series you want to average)
  vals_bf <- get_XH(bf)
  pr_outputs[[i]] <- vals_bf$true_pr# Replace with bf$whatever if you want another part
  time_outputs[[i]] <- bf$outputs$time
}
```

```{r}
# Suppose all vectors are of the same length and stored in all_outputs
all_runs <- length(pr_outputs)

# Convert list of vectors to a matrix
output_matrix <- do.call(rbind, pr_outputs)  # each vector becomes a row

# Take column-wise mean
av_baseline_focast <- colMeans(output_matrix)

```


```{r}
plot(time_outputs[[1]], av_baseline_focast, type = "l", lwd = 2, col = "blue",
...
```{r}
#FirstYear <- 2015
#t <- baseline_Adjumani$outputs$time / 365 + FirstYear
#vals_recon <- get_XH(recon_Adjumani)
#vals_bas <- get_XH(baseline_Adjumani)
 # df <- data.frame(
 #    Year = t,
 #    fit_model = round(vals_recon$true_pr,4),
 #    base = round(vals_bas$true_pr, 4)
 #  )
 #  df <- df[df$Year >= 2020,]

 # plot(t, vals_bas$true_pr, type = "l", lwd = 2, col = "blue",
  #     ylim = c(0, 1), ylab = "Parasite Rate (PR)", xlab = "Year",
   #    main = paste(dname))

  #lines(recon_Adjumani$outputs$time/365 + FirstYear, vals_recon$true_pr, col = "darkred", lwd = 2)
...
irs_rounds1$t_init <- c(3285,3650,4105,4380)
irs_rounds1$type <- c("actellic","actellic","actellic", "actellic")


forecast_Adjumani_itn_irs <- setup_irs(baseline_focast, coverage_name="multiround", coverage_opts=irs_rounds1, effect_sizes_name = "simple")
forecast_Adjumani_irs <- setup_irs(baseline_focast, coverage_name="multiround", coverage_opts=irs_rounds1, effect_sizes_name = "simple")

forecast_Adjumani_irs <-xds_solve(forecast_Adjumani_irs,5475,15)
...
forecast_Adjumani_itn_irs <- setup_bednets(forecast_Adjumani_itn_irs, coverage_name = "multiround", coverage_opts = bednets_rounds1, effect_sizes_name = "lemenach")
forecast_Adjumani_itn <- setup_bednets(baseline_focast, coverage_name = "multiround", coverage_opts = bednets_rounds1, effect_sizes_name = "lemenach")

forecast_Adjumani_itn_irs <- xds_solve(forecast_Adjumani_itn_irs,5475,15)
forecast_Adjumani_itn <-xds_solve(forecast_Adjumani_itn,5475,15)

```

```{r}
FirstYear <- 2015
vals_forecast_itn_irs <- get_XH(forecast_Adjumani_itn_irs)
vals_base_focast <- get_XH(baseline_focast)
vals_forecast_irs <- get_XH(forecast_Adjumani_irs)
vals_forecast_itn <- get_XH(forecast_Adjumani_itn)

index_itn <- which(baseline_focast$outputs$time == 4110)
...
index_irs <- which(baseline_focast$outputs$time == 3285)
...
mean_percent_reduction_itn_irs <- mean(percent_reduction_itn_irs, na.rm = TRUE)# (excluding any NaN/Inf from division by zero)
 ...
 plot(baseline_focast$outputs$time/365 + FirstYear, vals_base_focast$true_pr, type = "l", lwd = 2, col = "blue",
       ylim = c(0, 1), ylab = "Parasite Rate (PR)", xlab = "Year",
      main = paste(dname))
 lines(forecast_Adjumani_itn$outputs$time/365 + FirstYear, vals_forecast_itn$true_pr, col="darkorange", lwd=2)
#lines(forecast_Adjumani_irs$outputs$time/365 + FirstYear, vals_forecast_irs$true_pr, col="purple4", lwd=2)
lines(forecast_Adjumani_itn_irs$outputs$time/365 + FirstYear, vals_forecast_itn_irs$true_pr, col="darkred", lwd=2)
...
legend("topright",           # Position
       legend = c("baseline",legend_label1, legend_label2),  # Labels
       col = c("blue", "darkorange","darkred"),           # Line colors
       lty = 1,                          # Line type
       lwd = 2)        
```
```
