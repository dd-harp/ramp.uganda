...
itn_time <- list()
for (j in dist_name)
  itn_time[[j]] <- sort(get_itn_jdates(j, 2015))
```
...
#par(xpd = TRUE, mar = c(5, 4, 4, 2))  # allow plotting outside
  ...
  #itn_times <- c(FirstYear + 1049/365,FirstYear + 2114/365,FirstYear + 3148/365)
  itn_times <- c(FirstYear + 776/365,FirstYear + 2175/365,FirstYear + 3285/365)
  itn_labels <- c("ITNs2","ITNs3","ITNs4")
  # itn_times <- c(FirstYear + 2114/365,FirstYear + 3148/365)
  # itn_labels <- c("ITNs3","ITNs4")
  text(x = itn_times, y = rep(0.8, length(itn_times)),  # adjust y position as needed
  labels = itn_labels, pos=3, cex = 0.8,srt = 90, col = "blue")
  abline(v = itn_times, col = "darkred", lty = 1, lwd=2)        
...
```{r}
pdf("West_Nile_ITN.pdf")
malaria_itn_Effect <- list()

# Loop over all i values
for(i in c(118,128,146)) {
  itn <- profile_itn(i, fitted_mod, data_list)  # Get a data.frame
...
# Save to CSV
write.csv(malaria_itn_eff, "../data/malaria_itn_eff.csv", row.names = FALSE)
