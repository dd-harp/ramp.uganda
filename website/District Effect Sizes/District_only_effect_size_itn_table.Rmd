...
```{r}
pdf("permanent_districts_cts_only.pdf")
malaria_itn_Effect <- list()
Royal_guard  <- c(75,145,106,123,2,94,92,11,142,1,105,77,117,110,111,6)
Permanent_dual <- c(8,60,44)
Interceptor <- c(146,118,128,26,114,23,46,24,61,57,134,131,133,45,72)
cities <- c(10,33,35,37,42,86,95,99,101,140,98,27)
PBO <- setdiff(1:length(dist_name), c(Royal_guard,Permanent_dual,Interceptor,cities))
for(i in Permanent_dual) {
...
# Save to CSV
write.csv(malaria_itn_eff, "../data/permanent_itn_eff.csv", row.names = FALSE)
```
