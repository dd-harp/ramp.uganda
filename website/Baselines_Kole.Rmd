...
```{r, eval=F}
recon_Kole <- reconstruct_baseline(model, prts$pfpr, prts$jdate, irs_rounds, bednet_rounds, "mean")
```

```{r, eval=F}
saveRDS(recon_Kole, "recon_Kole.rds") 
```

```{r}
readRDS("recon_Kole.rds") -> recon_Kole
```


```{r}
xds_plot_PR(recon_Kole)
```
```{r}
baseline_Kole <- setup_irs(recon_Kole, coverage_name="multiround", coverage_opts=irs_rounds, effect_sizes_name = "simple")

baseline_Kole <- setup_bednets(baseline_Kole, coverage_name = "multiround", coverage_opts = bednet_rounds, effect_sizes_name = "lemenach")
```

```{r}
baseline_Kole <- xds_solve(baseline_Kole, 3650, 10)
```

```{r}
xds_plot_PR(recon_Kole)
xds_plot_PR(baseline_Kole, clrs = "darkred", add=T)
```
```{r}
baseline_focast <- baseline_Kole
...
forecast_Kole_itn_irs <- setup_irs(baseline_focast, coverage_name="multiround", coverage_opts=irs_rounds1, effect_sizes_name = "simple")
forecast_Kole_irs <- setup_irs(baseline_focast, coverage_name="multiround", coverage_opts=irs_rounds1, effect_sizes_name = "simple")

forecast_Kole_irs <-xds_solve(forecast_Kole_irs,5475,15)
...
forecast_Kole_itn_irs <- setup_bednets(forecast_Kole_itn_irs, coverage_name = "multiround", coverage_opts = bednets_rounds1, effect_sizes_name = "lemenach")
forecast_Kole_itn <- setup_bednets(baseline_focast, coverage_name = "multiround", coverage_opts = bednets_rounds1, effect_sizes_name = "lemenach")

forecast_Kole_itn_irs <- xds_solve(forecast_Kole_itn_irs,5475,15)
forecast_Kole_itn <-xds_solve(forecast_Kole_itn,5475,15)

```

```{r}
FirstYear <- 2015
vals_forecast_itn_irs <- get_XH(forecast_Kole_itn_irs)
vals_base_focast <- get_XH(baseline_focast)
vals_forecast_irs <- get_XH(forecast_Kole_irs)
vals_forecast_itn <- get_XH(forecast_Kole_itn)
...
mean_percent_reduction_itn_irs <- mean(percent_reduction_itn_irs, na.rm = TRUE)# (excluding any NaN/Inf from division by zero)
 ...
 plot(baseline_focast$outputs$time/365 + FirstYear, vals_base_focast$true_pr, type = "l", lwd = 2, col = "blue",
      ...
      main = paste(dname))
 lines(forecast_Kole_itn$outputs$time/365 + FirstYear, vals_forecast_itn$true_pr, col="darkorange", lwd=2)
#lines(forecast_Kole_irs$outputs$time/365 + FirstYear, vals_forecast_irs$true_pr, col="purple4", lwd=2)
lines(forecast_Kole_itn_irs$outputs$time/365 + FirstYear, vals_forecast_itn_irs$true_pr, col="darkred", lwd=2)
