...
percent_eff <- read.csv(file="C:/Users/doree/Box/RAMP/data/effect_sizes/ITN/malaria_type_itn_eff_2023.csv")
...
itn_file_only <- read.csv(file ="C:/Users/doree/Box/RAMP/data/effect_sizes/ITN/only_itn_eff.csv")
...
itn_mod1 <- setup_bednets(mod_file,
     coverage_name = "none", coverage_opts = cov_opts, 
     effectsizes_name = "none")

xds_solve(itn_mod1, max(tt)) -> itn_mod1
  
  vals_cov <- get_XH(itn_mod1)
vals_for <- as.data.frame(vals_for)
   time_years <- mod_for_cast$outputs$time/365+FirstYear
   vals_cov<- as.data.frame(vals_cov)
  df <- data.frame(
    Year = time_years,
    Original_PR = vals_for$true_pr,
    itn_PR = round(vals_for$true_pr, 4)
    #irs_PR_2024 = round(vals_cov$true_pr, 4)
  )
  df <- df[df$Year >= 2020,]
  ...
  plot(df$Year, df$Original_PR, type = "l", lwd = 2, col = "blue",
       ...
       main = paste(unique(data$district_name), "-", "cov=", 0.3*100,"%"))
