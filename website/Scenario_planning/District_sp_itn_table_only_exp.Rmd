...
for (j in 1:length(average_eff$mean_percent)) {
  ...
  effect_itn_start <- average_eff$mean_percent[[j]] / 100
  effect_itn_end <- 0.1
  decay_start <- 2023
  decay_end <- 2026
  ...
  effect_decay <- function(t, e0, e1, rate, t_start) {
  # Decays from e0 at t_start toward e1 over time
  e1 + (e0 - e1) * exp(-rate * (t - t_start))
}

  pr_with_exp_effect_decay <- function(t, pr0, e0, e1, rate, t_start, k_pr, t0_pr) {
  effect_t <- effect_decay(t, e0, e1, rate, t_start)
  ...
  pr_itn <- pr_with_exp_effect_decay(time_years, vals_for$true_pr, effect_itn_start, effect_itn_end, rate, t_s, k, t0)
  pr_itn_2026 <- pr_with_exp_effect_decay(time_years, pr_itn, effect_itn_start, effect_itn_end,rate, t_s+3, k, t01)
...
```{r}
pdf("itn_scenario_time_decay_exp.pdf")
