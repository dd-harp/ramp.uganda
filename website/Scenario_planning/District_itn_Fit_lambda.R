...
fit_one_district = function(i, model){
  with(data_list[[i]],
    return(pr2Lambda_history(pr, times, model))
)}
## ----fit all-----------------------------------------------------------------------------------------------------------
fitted_models=list()
...
Lt = function(t, pars){(1.01+sin(2*pi*t/365))^2}
model <- xds_setup(MYZname= "SI", Lname= "basicL", Xname= "SIP",Xopts = Xo)
model$Lpar[[1]]$Lambda <- 1000
model$Lpar[[1]]$season_par <- Lt
model$Lpar[[1]]$trend_par  <- Lt
tt <- seq(0, ceiling(max(data_list[[10]]$times) / 365) * 365 + (5 * 365), length.out = 200)
model<- xds_solve(model, max(tt))
xds_plot_PR(model)
for(i in 10)
