...
irs_time <- irs_time_full#[c(34)]# for a single district Torroro
```
...
vals <- get_XH(fitted_mod[[1]])
  with(vals, plot(time/365+FirstYear, true_pr, ylab = "PR",lwd=2, xlab = "Time", main=unique(data_list[[1]]$district_name), type = "l", ylim = c(0,1), col="darkred"))
  #with(vals, lines(time/365+FirstYear, true_pr, lwd=2,  col="darkblue"))
  with(data_list[[1]], lines(jdate/365+FirstYear, pfpr, pch=16,type = "o", col="darkblue"))
  add_irs_history(unique(data_list[[1]]$district_name))
```


# mean effect by IRS type
```{r}
irs_file <- read.csv(file="C:/Users/Admin/Box/RAMP/data/effect_sizes/IRS/merged_ cts_and_after_2021_irs_eff.csv")
average_eff <- aggregate(mean_percent ~ irs_Type, data = irs_file, FUN = mean)
```
...
for (j in 1:length(average_eff$mean_percent)) {

  # === Decay and PR calculation ===
  effect_irs_start <- average_eff$mean_percent[[j]] / 100
  effect_irs_end <- 0.01
  decay_start <- 2023
  decay_end <- 2024
  t0 <- (decay_start + decay_end) / 2
  t01 <- (decay_start + 1 + decay_end + 1) / 2
  k <- 1.5
...
#    effect_t <- effect_decay(t, e0, e1, k_eff, t0_eff)
  ...
  pr_irs <- logistic_pr_with_effect_decay(time_years, vals_for$true_pr, effect_irs_start, effect_irs_end, k, t0, k, t0)
  pr_irs_2024 <- logistic_pr_with_effect_decay(time_years, pr_irs, effect_irs_start, effect_irs_end, k, t01, k, t01)

  df <- data.frame(
    Year = time_years,
    Original_PR = vals_for$true_pr,
    irs_PR = round(pr_irs, 4),
    irs_PR_2024 = round(pr_irs_2024, 4)
  )
  df <- df[df$Year >= 2020,]
  df_first <- df[df$Year >= 2023 & df$Year <= 2024, ]
  df_sec   <- df[df$Year >= 2023.8, ]

  # === Plot for each ITN type ===
  plot(df$Year, df$Original_PR, type = "l", lwd = 2, col = "blue",
       ylim = c(0, 1),
       ylab = "Parasite Rate (PR)", xlab = "Year",
       main = paste(unique(data$district_name), "-", average_eff$irs_Type[[j]]))

  lines(df$Year, df$irs_PR, col = "darkred", lwd = 2)
  lines(df$Year, df$irs_PR_2024, col = "green", lwd = 2)
...
```{r}
pdf("irs_scenario_time_decay.pdf")
