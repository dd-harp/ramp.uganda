...
irs_time <- irs_time_full[c(31)]# for a single district Torroro
```
...
Get the fitted models for only irs_districts
```{r, eval=F}
fitted_mod <- list()
...
profile_irs = function(i, model_list, data_list){
  ...
  t_eff <-  seq(0, (5*365), length.out=200)
  cf_base <- make_cf_no_base(mod_file)
  mod1a <- modify_baseline(mod_file, cf_base) 
  mod1b <- last_to_inits(mod_file)
  mod1b <- forecast_spline_EIR(mod1b, 5,x_last=0)
  mod1b <- xds_solve_cohort(mod1b, times=t_eff)
  mod_scenario <- last_to_inits(mod_file)
  mod_scenario <- xds_solve_cohort(mod_scenario, times=t_eff)
  
  vals <- get_XH(mod_file)
  vals1a <- get_XH(mod1a)
  vals1b <- get_XH(mod1b)
  val_eff <- get_XH(mod_scenario)
  
   effect <- 0.2
   pr_eff <- val_eff$true_pr*(1-effect)
  with(vals1b, plot(time/365+2024.918, true_pr, ylab = "PR",lwd=2, xlab = "Time", main=unique(data$district_name), type = "l", ylim = c(0,1), col="darkred"))
  lines(val_eff$time/365 +2024.918 , pr_eff ,  lwd= 2, col="blue")
  #with(vals1b, lines(time/365+2024.918, true_pr, lwd=2,  col="green"))
